...
public class PV0 extends Protocol
{
	private int delay;
	private RuleInstance[] rules;
	private byte[] clientJAR;

	public PV0(DataInputStream netIn, DataOutputStream netOut, int delay, RuleInstance[] rules, byte[] clientJAR)
	{
		this.netIn = netIn;
		this.netOut = netOut;
		this.delay = delay;
	...
	private void sendRule(RuleInstance rule) throws IOException
	{
		...
		writeString(new JSONArray(rule.ignoreFiles).toString()); //发送远程客户端用的忽略文件摘要y
